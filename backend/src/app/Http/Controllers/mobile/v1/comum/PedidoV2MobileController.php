...
/**
 * @property Pedido $model
 */
class PedidoV2MobileController extends BaseMobileController
{
    private $tenant;
    protected $className;
    ...
    public function pedidojson(Request $request)
    {
        try {
            ...
            //VERIFICA SE O JSON É INVÁLIDO
            if (is_null($array)) {
                $resultado = $this->retorna_msg_erro(ERRO_JSON_INVALIDO, "", $array, "json");
                goto mensagem;
            }
            ...
            foreach ($array as $pedido_completo) {
                ...
                if (!isset($validacao_pedido[0]) && $i === NULL) {
                    ...
                    $atualizarPedidoEItem = TRUE;

                    //se existir registro ele atualiza
                    if ($existe != NULL && $existe["status"] == 0) {
                    ...
                    } elseif (!is_null($existe) && $existe["status"] == 1) {
                        $pedidoRetornoInfo = [
                            "id_mobile" => $id_pedido_mobile,
                            "id_nuvem" => $existe["id"],
                            "id_retaguarda" => $existe["id_retaguarda"],
                            "status" => $existe["status"],
                            "dt_sinc_erp" => $existe["dt_sinc_erp"]
                        ];
                        $atualizarPedidoEItem = FALSE;
                ...
                if (count($erros) > 0) {
                    ...
                    if (count($pedidoRetornoInfo) > 0) {
                        $infoPedido = $pedidoRetornoInfo;
                    } else {
                        $infoPedido = [
                            "id_mobile" => $id_pedido_mobile,
                        ...
                        // dispacha o pedido para fila
                        EnviaPedidoErpJob::dispatch($this->tenant, $id_interno_pedido)->onQueue("pedido");
                    }

                    $this->_enviarNotificacao($infoPedido, $pedido_completo["pedido"]["mac"]);
                    $retorno[] = $infoPedido;
                }
            }

            if ((count($erros) > 0) && ($qtd_pedido_inserido > 0)) {
                $resultado = [
            ...
            } elseif ((count($erros) > 0) && ($qtd_pedido_inserido === 0)) {
                $resultado = [
                    'code' => HTTP_NOT_FOUND,
                    'status' => 'error',
            ...
            return response()->json(['erro' => true, 'message' => $ex->getMessage()], 400);
        }
    }
    ...
    {
        $pedido = $this->model::select(
            "id",
            "id_retaguarda",
            "dt_sinc_erp",
            DB::raw("IF(id_retaguarda IS NULL,0,1) AS status")
        )
            ->where([
                "id_pedido_dispositivo" => $idPedidoMobile,
                "id_vendedor" => $idVendedor,
                "origem" => ORIGEM_PEDIDO_AFV
            ])->first();
    ...
    protected function _enviarNotificacao($infoPedido, $mac)
    {
        $devices = Dispositivo::select("token_push")->where("mac", "LIKE", "%" . $mac . "%")->first();

        if (!is_null($devices)) {
            $tokenPush = [
                "a" => [
                    "token_push" => $devices["token_push"]
                ]
            ];

            $payload = array(
                "acao" => "pedidonuvem",
                "id_nuvem" => $infoPedido["id_nuvem"],
                "id_mobile" => $infoPedido["id_mobile"],
                "status" => $infoPedido["status"]
            );

            $fcm = new FcmService;
            $notification = $fcm->notificationGeneric($payload, $tokenPush);

            //Enviando notificação para o dispositivo
            $fcm->send_notification($notification);
        }
    }
}
