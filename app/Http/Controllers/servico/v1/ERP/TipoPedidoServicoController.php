...
class TipoPedidoServicoController extends BaseServicoController
{
    ...
    public function __construct()
    {
        ...
        $this->filha = $this;
        $this->metodo = 'Tipopedido' . CLASS_SERVICE;
        $this->entity = TipoPedidoServicoController::class;
    ...
    public static function atualizarDados($array, $erroPacote)
    {
        try {
            ...
            $vetor = [
                "tipoPedido" => $service->getAllId("tipo_pedido", TRUE)
            ];
            ...
            if (!is_null($erroValidacaoIds)) {
                ...
                for ($i = 0; $i < $totalRegistros; $i++) {
                    ...
                    if ($validacao["status"] == "sucesso") {
                        ...
                        $idTipoPedido = isset($vetor["tipoPedido"][$item["id_retaguarda"]]) ? $vetor["tipoPedido"][$item["id_retaguarda"]] : NULL;

                        $item["status"] = 1;
    
                        if (!is_null($idTipoPedido)) {
    ...
    {
        return parent::store($dados, TipoPedido::class, ["id_retaguarda"], self::_nomeCamposDb());
    }

    public function atualizarInserirVetores($dados)
    {
        return parent::storeVetores($dados, TipoPedido::class, ["id_retaguarda"], self::_nomeCamposDb());
    }

    public function _nomeCamposDb()
    {
        return array("id_retaguarda", "descricao");
    }
}